...
import cache from '../utils/cache'; // for cache invalidation
...
// POST /api/agent/register (merged: handles both with and without registration token)
router.post('/register', async (req, res) => {
    try {
        ...
        const device = new Device({
            ...
            version,
            token,
            userId,          // ← assigned if token was valid
            status: 'online',
            lastSeen: new Date(),
        });
        await device.save();
...
router.post('/results', authenticateAgent, async (req, res) => {
    try {
        ...
        // Invalidate caches that depend on latest data
